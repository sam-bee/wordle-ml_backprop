...
policy[p] = trunk_b2[p] + sum_k trunk_w2[p, k] * hidden1[k]
```
...
| `dense_trunk.hidden1_to_output` | `[64][128]` | `[64]` | none |
...
9. `dense_trunk.hidden1_to_output.weights` - `64 * 128 = 8,192` fp16 values
10. `dense_trunk.hidden1_to_output.biases` - `64` fp16 values
...
| Dense trunk | 123,584 | 247,168 |
| Policy model total | 150,528 | 301,056 |
...
```text
offset 0:
    PolicyModelParameters, 301,056 bytes
...
```text
genome_byte_count = 301056 + action_count * 76
```
